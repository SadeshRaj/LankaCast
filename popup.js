...
// --- SECURE DOM CREATION HELPER (Hero) ---
...
// --- SECURE DOM CREATION HELPER (List Item) ---
function createNewsCard(item) {
    ...
    // Whatsapp Object/Button Hack (Preserved from original)
...
function renderAlertsData(historyItems) {
    ...
    if (historyItems && historyItems.length > 0) {
        ...
        container.innerHTML = '<div class="empty-state">No alerts triggered yet.</div>';
    }
}
...
function showSkeletons(containerId) {
    ...
    // Skeletons are static layout, innerHTML is fine here as no user input is involved
    container.innerHTML = `
...
function forceRefresh() {
    ...
    // Wait for actual response from background
    chrome.runtime.sendMessage({ action: "refreshNews" }, (response) => {
        // Even if background is instant, give a tiny visible delay for UX, then load
        loadDataAsync();
        setTimeout(() => {
