...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // FEATURE 1: Initialize split toggle bindings[cite: 3]
    const masterToggle = document.getElementById('notifToggle');
    const siToggle = document.getElementById('sinhalaToggle');
    const enToggle = document.getElementById('englishToggle');
    const taToggle = document.getElementById('tamilToggle');

    if(masterToggle) {
        masterToggle.addEventListener('change', (e) => {
            const isChecked = e.target.checked;
            chrome.storage.local.set({ notificationsEnabled: isChecked });
            toggleSubPanelState(isChecked);
        });
    }
    if(siToggle) siToggle.addEventListener('change', (e) => chrome.storage.local.set({ sinhalaNotifEnabled: e.target.checked }));
    if(enToggle) enToggle.addEventListener('change', (e) => chrome.storage.local.set({ englishNotifEnabled: e.target.checked }));
    if(taToggle) taToggle.addEventListener('change', (e) => chrome.storage.local.set({ tamilNotifEnabled: e.target.checked }));
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        ...
        if (changes.tamilNews) {
            renderNewsData(changes.tamilNews.newValue, 'tamil-hero', 'tamil-list', 'tamil-loader');
        }
    });
});

function toggleSubPanelState(isEnabled) {
    const panel = document.getElementById('subSettingsPanel');
    if (!panel) return;
    if (isEnabled) {
...
function loadDataAsync() {
    chrome.storage.local.get(
        ['sinhalaNews', 'englishNews', 'tamilNews', 'alertHistory', 'keywords', 'notificationsEnabled', 'sinhalaNotifEnabled', 'englishNotifEnabled', 'tamilNotifEnabled'],
        (data) => {
            ...
            renderNewsData(data.tamilNews, 'tamil-hero', 'tamil-list', 'tamil-loader');
            ...
            if(masterToggle && data.notificationsEnabled !== undefined) {
                masterToggle.checked = data.notificationsEnabled;
                toggleSubPanelState(data.notificationsEnabled);
            }
            if(siToggle && data.sinhalaNotifEnabled !== undefined) siToggle.checked = data.sinhalaNotifEnabled;
            if(enToggle && data.englishNotifEnabled !== undefined) enToggle.checked = data.englishNotifEnabled;
            if(taToggle && data.tamilNotifEnabled !== undefined) taToggle.checked = data.tamilNotifEnabled;
        }
    );
}
...
function createHeroCard(item) {
    ...
    container.className = "hero-card-wrapper";
    ...
    topMetaRow.className = "hero-top-row";

    const badge = document.createElement('span');
    badge.className = "hero-badge";
    badge.textContent = "Latest";

    // FEATURE 3: Floating WhatsApp button inside the Hero template
    ...
    overlay.appendChild(topMetaRow);
...
function setupNav() {
    const tabs = ['btn-sinhala', 'btn-english', 'btn-tamil', 'btn-alerts'];
    const views = ['view-sinhala', 'view-english', 'view-tamil', 'view-alerts'];
